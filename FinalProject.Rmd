    ...
    if (temp != 6) {
      for (y in temp:5) {
        if (y == x) {
          ...
          saltlakestoredistance <- distm(c(saltlakecitydata$longitude[x], saltlakecitydata$latitude[x]), c(saltlakecitydata$longitude[y], saltlakecitydata$latitude[y]), fun = distHaversine);
    ...
    if (temp != 6) {
      for (y in temp:5) {
        if (y == x) {
          ...
          loganstoredistance <- distm(c(logandata$longitude[x], logandata$latitude[x]), c(logandata$longitude[y], logandata$latitude[y]), fun = distHaversine);
          loganavgdist = loganavgdist + loganstoredistance;
        }
      }
    }
}
...
```{r}
slcdistances <- lapply(distm(c(saltlakecitydata$longitude[x], saltlakecitydata$latitude[x]), c(saltlakecitydata$longitude[y], saltlakecitydata$latitude[y]), fun = distHaversine))
