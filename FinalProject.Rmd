...
barplot(all_avg_distances, 
        names.arg = c("SLC", "Logan", "Provo", "Orem", "S. Jordan", "Farmington", "Wellington", "Morgan", "Roosevelt", "Woods Cross"), 
        xlab = "Cities",
        ylab = "Distance in Meters (m)", 
        main = "Average Distance Between Stores In Cities", 
        col=c("#1b98e0", "#1b98e0", "#1b98e0", "#1b98e0", "#1b98e0",  "#111111", "#111111", "#111111", "#111111", "#111111"), las = 3)
legend("topright",  # add legend to bar plot
       legend = c("Urban Cities", "Rural Cities"),
       fill = c("#1b98e0", "#111111"))
...
Average Urban Store Distance:
```{r}
urbancitiesdist<- (mean(slcdist) + mean(logandist) + mean(provodist) + mean(oremdist) + mean(sjdist))/5
urbancitiesdist
```
Average Rural Store Distance:
```{r}
ruralcitiesdist<- (mean(farmingtondist)+ mean(wellingtondist) + mean(morgandist)+ mean(rooseveltdist)+ mean(woodscrossdist))/5
ruralcitiesdist
```
...
Store types help us look at what is available in the area. By looking to see if a city has more convience stores or grocery stores, we can get insight into what is mostly accessible in the area. A higher number of convenience stores also contribute to food deserts.
